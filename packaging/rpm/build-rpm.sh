...
project_version() {
  sed -n "s/^[[:space:]]*version[[:space:]]*:[[:space:]]*'\\([^']*\\)'.*/\\1/p" "$PROJECT_DIR/meson.build" | head -n 1
}
...
spec="$PROJECT_DIR/packaging/rpm/gstreamer.spec"
archive="$TOPDIR/SOURCES/${name}-${commit}.tar.gz"

if [[ -z "$version" ]]; then
  echo "Could not determine gstreamer version from meson.build" >&2
  exit 1
fi

echo "==> $name $version-$release ($shortcommit)"
